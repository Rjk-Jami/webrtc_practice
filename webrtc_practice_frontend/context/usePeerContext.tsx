...
interface PeerContextType {
  ...
  createAnswer: (offer: RTCSessionDescriptionInit) => Promise<RTCSessionDescriptionInit | void>
...
export const PeerProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const setRemoteAnswer = useCallback(async (ans: RTCSessionDescriptionInit) => {
    if (!peer) return

    // Can only set remote answer when in have-local-offer state (we created an offer and are waiting for answer)
    // if (peer.signalingState !== "have-local-offer") {
    //   console.log("Cannot set remote answer, signaling state:", peer.signalingState)
    //   return
    // }
  ...
  return (
    <PeerContext.Provider
      value={{ peer, createOffer, createAnswer, setRemoteAnswer, sendStream, resetPeer }}
    >
      {children}
    </PeerContext.Provider>
  )
